...
def get_train_tfrecord(imgs_path, data_entry_path, data_label_path, split_list_path, image_size=224, label_num=15, mode='train', D=40):
    ...
    for i in range(D):
        #oviya added the round function below!
        subsets_num = round(split_id_list.__len__() / D) + 1
        sub_split_id_list = split_id_list[i * subsets_num: (i + 1) * subsets_num]

        tfrecord_name = 'data/tfrecord/' + mode + '-%02d.tfrecord' % i
        # oviya changed this below. tf.python_io.TFRecordWriter(tfrecord_name)
        ...
        for image_index in sub_split_id_list:
            ...
            for label in labels:
                for j in range(label_num):
                    ...
                    if label == oviyalabel:
                    # oviya changed this above: if label == finding_labels[j]:
                        label_one_hot[j] = 1
...
def main():
    # '/home/wanglei/workshop/NIHChestX-Ray/images'
    imgs_path = '/Users/oviyat/Desktop/cx/theimages'
